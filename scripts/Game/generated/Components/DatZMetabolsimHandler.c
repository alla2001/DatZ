...
class DatZMetabolsimHandler : ScriptComponent
{
	...
	static const int INFECTION_CHANCE = 20;
    ...
    void update()
    {
		...
		// Delta-based replication: only send update if values changed significantly
		float infectionProgress = GetInfectionProgress();

		bool shouldSendUpdate = false;

		// Check if any value changed beyond threshold
		if (Math.AbsFloat(food - m_fLastSentFood) >= FOOD_WATER_THRESHOLD)
			shouldSendUpdate = true;
		else if (Math.AbsFloat(water - m_fLastSentWater) >= FOOD_WATER_THRESHOLD)
			shouldSendUpdate = true;
		else if (Math.AbsFloat(stamina - m_fLastSentStamina) >= STAMINA_THRESHOLD)
			shouldSendUpdate = true;
		else if (hpStage != m_iLastSentHpStage)
			shouldSendUpdate = true;
		else if (m_bIsInfected != m_bLastSentInfected)
			shouldSendUpdate = true;
		else if (m_bIsInfected && Math.AbsFloat(infectionProgress - m_fLastSentInfectionProgress) >= INFECTION_THRESHOLD)
			shouldSendUpdate = true;
		else if (m_bZombieHitEffect != m_bLastSentZombieHitEffect)
			shouldSendUpdate = true;

		// Only send RPC if something meaningful changed
		if (shouldSendUpdate)
		{
			m_fLastSentFood = food;
			m_fLastSentWater = water;
			m_fLastSentStamina = stamina;
			m_iLastSentHpStage = hpStage;
			m_bLastSentInfected = m_bIsInfected;
			m_fLastSentInfectionProgress = infectionProgress;
			m_bLastSentZombieHitEffect = m_bZombieHitEffect;
	 ...
	 void RPC_Eat(float amount)
    {
		...
		Replication.BumpMe();
		ForceSendUpdate(); // Immediate feedback to client
    }
	...
	// Force send update to client (bypasses delta check, used for important events)
	private void ForceSendUpdate()
	{
		if (!Replication.IsServer())
			return;

		// Calculate hpStage for immediate update
		int hpStage = 0;
		if (food <= 0.0) hpStage--;
		if (water <= 0.0) hpStage--;
		if (food > maxFood * 0.5 && water > maxWater * 0.5) hpStage++;
		if (food > maxFood * 0.7 && water > maxWater * 0.7) hpStage++;
		if (food > maxFood * 0.8 && water > maxWater * 0.8) hpStage++;
	...
	// Delta-based replication: track last sent values to avoid unnecessary network traffic
	private float m_fLastSentFood = -1;
	private float m_fLastSentWater = -1;
	private float m_fLastSentStamina = -1;
	private int m_iLastSentHpStage = -99;
	private bool m_bLastSentInfected = false;
	private float m_fLastSentInfectionProgress = -1;
	private bool m_bLastSentZombieHitEffect = false;

	// Thresholds for replication - only send when change exceeds these values
	private static const float FOOD_WATER_THRESHOLD = 1.0;    // Send if changed by 1 unit
	private static const float STAMINA_THRESHOLD = 5.0;       // Send if changed by 5 units
	private static const float INFECTION_THRESHOLD = 0.01;    // Send if changed by 1%
