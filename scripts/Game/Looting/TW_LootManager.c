...
sealed class TW_LootManager
{
	...
	private ref LootManagerSettings m_Settings = new LootManagerSettings();
	...
	// Maps faction key to DatZLootFaction flag
	static int GetFactionFlag(string factionKey)
	{
		factionKey.ToLower();
		if (factionKey.Contains("us") || factionKey.Contains("usa"))
			return DatZLootFaction.US;
		if (factionKey.Contains("ussr") || factionKey.Contains("soviet"))
			return DatZLootFaction.USSR;
		if (factionKey.Contains("fia") || factionKey.Contains("resistance") || factionKey.Contains("guerilla"))
			return DatZLootFaction.FIA;
		// Default civilian for unknown factions
		return DatZLootFaction.CIVILIAN;
	}

	// Returns array of [entry, factionFlag] pairs for faction-aware loot building
	void GetFactionCatalogsWithFlags(notnull array<ref array<SCR_EntityCatalogEntry>> outEntries, notnull array<int> outFactionFlags)
	{
		...
		foreach(Faction fac : factions)
		{
			...
			if(!itemCatalog)
			{
				PrintFormat("TrainWreckLooting: Faction does not have an item catalog: %1", WidgetManager.Translate(faction.GetFactionName()), LogLevel.WARNING);
				continue;
			}

			ref array<SCR_EntityCatalogEntry> current = new array<SCR_EntityCatalogEntry>();
			itemCatalog.GetEntityList(current);

			if (current.IsEmpty())
				continue;

			// Determine faction flag from faction key
			string factionKey = faction.GetFactionKey();
			int factionFlag = GetFactionFlag(factionKey);

			Print(string.Format("[TW_LootManager] Faction %1 (%2) -> flag %3, items: %4",
				WidgetManager.Translate(faction.GetFactionName()), factionKey, factionFlag, current.Count()), LogLevel.NORMAL);

			outEntries.Insert(current);
			outFactionFlags.Insert(factionFlag);
		}
	}
	...
	{
		ref array<ref array<SCR_EntityCatalogEntry>> factionEntries = {};
		ref array<int> factionFlags = {};
		GetFactionCatalogsWithFlags(factionEntries, factionFlags);

		ref array<SCR_EntityCatalogEntry> entries = {};
		foreach (array<SCR_EntityCatalogEntry> factionItems : factionEntries)
		{
			entries.InsertAll(factionItems);
		}
	...
	// REDUCED VALUES for DayZ-style hardcore survival gameplay
	static int GetBalancedSpawnChance(SCR_EArsenalItemType itemType)
	{
		switch (itemType)
		{
			// EXCLUDED - Should not spawn (handled by ExcludeItemType)
			...
			// COMMON - Civilian clothing & basic items (reduced 25-30%)
			...
			// FOOD - Common survival items (canned food, water, rations)
			case SCR_EArsenalItemType.FOOD:
				return 25;

			// INDUSTRIAL - Tools, building materials, work items
			case SCR_EArsenalItemType.INDUSTRIAL:
				return 20;

			// AMMO - Loose magazines and ammunition
			case SCR_EArsenalItemType.AMMO:
				return 18;
	...
	void InitializeLootTable()
	{
		...
		// Exclude overpowered/inappropriate items for DayZ-style survival
		...
		// Get faction catalogs with faction flags for tier-based filtering
		ref array<ref array<SCR_EntityCatalogEntry>> factionEntries = {};
		ref array<int> factionFlags = {};
		GetFactionCatalogsWithFlags(factionEntries, factionFlags);

		if (factionEntries.IsEmpty())
		{
			Print("[TW_LootManager] No catalog items found - faction manager may not be ready", LogLevel.WARNING);
			return;
		}

		int totalItems = 0;

		// Process each faction's catalog with its faction flag
		for (int factionIdx = 0; factionIdx < factionEntries.Count(); factionIdx++)
		{
			array<SCR_EntityCatalogEntry> catalogItems = factionEntries[factionIdx];
			int currentFactionFlag = factionFlags[factionIdx];
			...
			foreach(auto entry : catalogItems)
			{
				...
				foreach(auto data : itemData)
				{
					...
					// Pass faction flag to SetData
					config.SetData(prefab, defaultChance, defaultCount, null, arsenalItem.ShouldSpawn(), currentFactionFlag);
		...
		Print(string.Format("[TW_LootManager] Loaded %1 items from %2 factions with faction flags", totalItems, factionEntries.Count()), LogLevel.NORMAL);
		...
		// Setup respawn listeners now that settings are loaded
		SetupRespawnListeners();
	}
	...
	{
		// Delay loot table init - faction manager might not be ready yet
		GetGame().GetCallqueue().CallLater(InitializeLootTable, 1000, false);
	}

	private void SetupRespawnListeners()
	{
		ref TW_MonitorPositions monitor = TW_MonitorPositions.GetInstance();
		if (!monitor)
			return;

		if(m_Settings && m_Settings.RespawnSettings && m_Settings.RespawnSettings.IsLootRespawnable)
		{
	...
	void TrickleSpawnLootInContainer(TW_LootableInventoryComponent container, int remainingAmount)
	{
		...
		// OPTIMIZATION: Use cached pool with faction filter
		WeightedType<ref TW_LootConfigItem> pool = GetCachedLootPool(container.GetTypeFlags(), container.GetFactionFilter());
	...
	//! Spawn loot in designated container - DayZ-style: most containers nearly empty
	void SpawnLootInContainer(TW_LootableInventoryComponent container)
	{
		if(!m_Settings.IsLootEnabled || !container)
			return;

		// Item count: 40% = 1 item, 35% = 2 items, 25% = 3 items
		int spawnCount;
		...
		for(int i = 0; i < spawnCount; i++)
		{
			...
			// Only spawn 1 copy of each item (no stacking)
	...
	// factionFilter: DatZLootFaction flags - only include items from these factions (0 = all factions)
	static WeightedType<ref TW_LootConfigItem> GetCachedLootPool(SCR_EArsenalItemType flags, int factionFilter = 0)
	{
		// Cache key combines type flags and faction filter
		// Use high bits for faction to avoid collision with type flags
		int flagKey = flags | (factionFilter << 20);
		...
		for(int i = 0; i < s_ArsenalItemTypes.Count(); i++)
		{
			...
			for(int j = 0; j < entries.Count(); j++)
			{
				...
				// Faction filter: 0 means all factions allowed
				// Otherwise, item must match at least one allowed faction flag
				if (factionFilter != 0 && item.factionFlags != 0)
				{
					if ((item.factionFlags & factionFilter) == 0)
