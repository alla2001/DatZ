...
class TW_LootableInventoryComponent : ScriptComponent
{
	...
	// Zone override: when set, this overrides m_LootItemTypes
	private bool m_bHasZoneOverride = false;
	private SCR_EArsenalItemType m_ZoneOverrideTypeFlags;
	private float m_fClosestOverrideDistSq = -1;
	private int m_iFactionFilter = 0; // DatZLootFaction flags (0 = all factions)
	...
	SCR_EArsenalItemType GetTypeFlags()
	{
		if (m_bHasZoneOverride)
			return m_ZoneOverrideTypeFlags;

		// If container has no specific types set (0 = all), default to coastal tier
		if (m_LootItemTypes == 0)
			return DatZLootSpawner.DATZ_DEFAULT_TYPE_FILTER;

		return m_LootItemTypes;
	}
	SCR_EArsenalItemMode GetModeFlags() { return m_LootItemModes; }

	//! Override the type flags from a loot zone (closest zone wins)
	//! distSq: squared distance to zone center
	//! factionFilter: DatZLootFaction flags (0 = all factions)
	void SetZoneTypeFilter(int typeFilter, float distSq = 0, int factionFilter = 0)
	{
		...
		m_bHasZoneOverride = true;
		m_ZoneOverrideTypeFlags = typeFilter;
		m_iFactionFilter = factionFilter;

		// Invalidate cached loot pools since filters changed
		TW_LootManager.InvalidateLootPoolCache();
	}

	//! Get the faction filter for this container
	int GetFactionFilter() { return m_iFactionFilter; }
